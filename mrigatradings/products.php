...
<div class="col-sm-3" style="float:right;">
	...
	<section>
       ...
       <?php //print_r($_POST['productids']);
	   if(!empty($_POST['productids'])) {
	   $txt_1=array_chunk($_POST['productids'], 7);
	   // count($txt_1);
	  // print_r($txt_1);
	   ?>
       
       <?php $k=1; for($i=0;$i<count($txt_1);$i++){?>
       <ul class="breadcrumb">
       		<?php if($k==1){?>
			<li><a href="index.php">Home</a></li><li></li><?php } ?>
            
		 	<?php $v_v=$txt_1[$i]; foreach($v_v as $selected_id) {?>
			<li><a href="javascript:void(0)"><?php echo productdetails($selected_id);?></a></li>
            <?php   } ?>
        ...
        <ul class="breadcrumb">       		 
            ...
            <?php  if(!empty($_REQUEST['productids'])) {?>
            <li><a href="javascript:void(0)"><?php echo productdetails($_REQUEST['productids']);?></a></li>
            <?php }?>
		</ul>  
        <?php } ?>
						...
						<?php 						
							...
							// Loop to store and display values of individual checked checkbox. 
								foreach($_POST['productids'] as $selected_id)
								{								
							$sql_list=mysql_query("select * from product_variety where product_id='".$selected_id."'") or die(mysql_error());					 
							if(mysql_num_rows($sql_list)==0){ echo "No product selected!..."; } else{
                        ...
                        <?php } //End If ?>
                         <?php } else { ?>
                         <?php
							$sql_list=mysql_query("select * from product_variety where product_id='".$_REQUEST['productids']."'") or die(mysql_error());					 
							if(mysql_num_rows($sql_list)==0){ echo "No product selected!..."; } else{
							while($rs=mysql_fetch_array($sql_list)){                         
						 ?>
                         <div class="col-sm-12">
